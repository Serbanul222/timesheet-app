...
// Local ExportState interface since it's not exported from the hook
interface ExportState {
  ...
  lastExport?: {
    ...
    filename: string
    timestamp: string
  } | null // Allow null to match the hook's return type
}
...
const PanelHeader = React.memo(
  ({
    ...
    onRefresh: () => void
    isLoading: boolean
    lastExport: ExportState['lastExport'] // This now includes null
  }) => (
    <div className="flex items-center justify-between mb-6">
      ...
      <div className="flex items-center gap-4">
        {lastExport && ( // This handles both null and undefined
    ...
    const loadPeriods = useCallback(async () => {
      ...
      if (data) {
        ...
        data.forEach(ts => {
          ...
          period.count++
          // Fix: Proper null checking and type assertion for Supabase response
          ...
          <div className="border rounded-lg max-h-64 overflow-y-auto">
            {periods.length > 0 ? (
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <div className="text-xs text-blue-600 mt-1">
                          {period.count} timesheets • {period.stores.length} stores
                        </div>
                      </div>
                      {selectedIndex === index && (
              ...
              <div className="p-6 text-center text-gray-500 text-sm">
                <p>No timesheet periods found.</p>
              </div>
            )}
          </div>
        )}
...
const ConfigPanel = React.memo(
  ({
    ...
    availableFormats: readonly ExportFormat[] // Fix: Accept readonly array
    ...
    const formatDetails = {
      excel: {
        ...
        desc: 'Grid format with time intervals'
      },
      csv: {
      ...
      <div className="space-y-6">
            ...
            <label className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Include employee notes in the export.
                </p>
              </div>
            </label>
            <label className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Include days with no recorded hours.
                </p>
              </div>
            </label>
          </div>
        ...
        <div>
          <Input
            ...
            disabled={disabled}
            helperText="Don't include file extension."
          />
        </div>
      </div>
    )
  ...
  ({
    ...
    <div className="space-y-6">
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 space-y-2 text-sm">
        ...
        <div className="flex justify-between">
          <span className="text-blue-700 font-medium">Days:</span>
          <span>{getDaysCount(dateRange)}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-blue-700 font-medium">Notes:</span>
          <span>{options.includeNotes ? 'Yes' : 'No'}</span>
        </div>
      </div>
      ...
      {exportState.isLoading && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          ...
          <p className="text-xs text-blue-700 mt-2">{exportState.statusText || 'Processing...'}</p>
        </div>
      )}
      ...
      >
        {exportState.isLoading
          ? `Generating... ${exportState.progress || 0}%`
...
export default function ExportPanel({ userRole }: ExportPanelProps) {
  ...
  // Handle export using the internal hook method
  const handleExport = useCallback(async () => {
    // Build export options with proper typing for exactOptionalPropertyTypes
    const baseOptions = {
      dateRange,
    ...
    // Only add filename if it has a value (not empty string)
