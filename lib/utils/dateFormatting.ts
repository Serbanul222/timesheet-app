...
/**
 * Format date with Romanian month name (e.g., "31 Iulie 2025")
 */
export const formatDateRomanian = (date: Date | string | null | undefined): string => {
  if (!date) {
  ...
  return format(dateObj, 'dd MMMM yyyy', { locale: ro });
}

/**
 * Format month and year in Romanian (e.g., "Iulie 2025")
 */
export const formatMonthYearRomanian = (date: Date | string | null | undefined): string => {
  if (!date) {
    return 'Dată invalidă';
  }
  ...
  return format(dateObj, 'MMMM yyyy', { locale: ro });
}
...
/**
 * Get month range display with Romanian month names (e.g., "1 - 31 August 2025").
 */
export const getMonthRangeDisplay = (date: Date): string => {
  ...
  const monthName = format(firstDay, 'MMMM yyyy', { locale: ro });
  return `1 - ${lastDay.getDate()} ${monthName}`;
}

/**
 * Get month range display in Romanian with full formatting
 */
export const getMonthRangeDisplayRomanian = (date: Date): string => {
  const year = date.getFullYear();
  const month = date.getMonth();
  const firstDay = new Date(year, month, 1);
  const lastDay = new Date(year, month + 1, 0);
...
/**
 * Get period display with Romanian month names for titles
 */
export const getPeriodDisplayRomanian = (startDate: Date | string, endDate: Date | string): string => {
  const start = typeof startDate === 'string' ? new Date(startDate) : startDate;
  const end = typeof endDate === 'string' ? new Date(endDate) : endDate;
  
  // If same month, show "Iulie 2025"
  if (start.getMonth() === end.getMonth() && start.getFullYear() === end.getFullYear()) {
    return formatMonthYearRomanian(start);
  }
  
  // If different months, show full range
  return `${formatDateRomanian(start)} - ${formatDateRomanian(end)}`;
}
...
export const getDefaultPeriodFormatted = () => {
  ...
  return {
    ...
    displayRomanian: getPeriodDisplayRomanian(startOfMonth, endOfMonth),
    monthRange: getMonthRangeDisplay(startOfMonth),
    monthRangeRomanian: getMonthRangeDisplayRomanian(startOfMonth),
